...
def train_disease_classifier():
    ...
    try:
        ...
        print(f"[ERROR] Dataset not found. Please ensure training_data.csv is in {TRAINING_DATA_PATH}.")
        return None, [], [], {}, {}

# Initialize system globally
ML_MODEL, FEATURE_COLS, TARGET_CLASSES, SYMPTOM_BASE_SCORES, CRITICAL_SYMPTOMS = None, [], [], {}, {}

def ensure_model_initialized():
    global ML_MODEL, FEATURE_COLS, TARGET_CLASSES, SYMPTOM_BASE_SCORES, CRITICAL_SYMPTOMS
    if ML_MODEL is None:
...
def analyze_symptom_rules(symptom_data: Dict[str, Any]) -> Tuple[float, List[str], str]:
    ...
    severity = symptom_data.get("severity", "moderate").lower()
    days = symptom_data.get("days", 1)
...
def determine_triage_level(score: int) -> Tuple[str, str]:
    if score <= 25:
        return "LOW", "Self-care advice: Hydration, rest, and monitoring."
    elif score <= 50:
        return "MODERATE", "Recommend teleconsultation. Suggest contacting local health worker."
    elif score <= 75:
        return "HIGH", "Visit hospital within 24 hours."
    else:
        return "CRITICAL", "Emergency medical attention immediately!"
...
def hybrid_clinical_triage_engine(patient_data: Dict[str, Any]) -> str:
    ...
    result = {
        ...
        "detected_symptoms": resolved_symptom_list
    }
